package uniandes.fabricasw.ecotouring.cli;
...
public class RenderCommand extends ConfiguredCommand<EcoTouringConfiguration> {
	...
	public void configure(Subparser subparser) {
		...
		subparser.addArgument("-i", "--include-default").action(Arguments.storeTrue()).dest("include-default")
				.help("Also render the template with the default name");
		subparser.addArgument("names").nargs("*");
	}

	@Override
	protected void run(Bootstrap<EcoTouringConfiguration> bootstrap, Namespace namespace,
			EcoTouringConfiguration configuration) throws Exception {
